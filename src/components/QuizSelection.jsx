...
import { fetchTopics, submitScore } from '../api';
...
const QuizSelection = () => {
  ...
  const fetchQuizData = async () => {
    try {
      ...
      const response = await fetchTopics(); // Replace with dynamic topic
      if (response.data && response.data.success) {
        const questions = response.data.data;
        setQuizData({
          title: questions[0]?.topicName || 'Quiz', // Initial title
  ...
  // Update title when current question changes
  useEffect(() => {
    if (quizData.questions.length > 0 && currentQuestion < quizData.questions.length) {
      setQuizData(prevData => ({
        ...prevData,
        title: prevData.questions[currentQuestion]?.topicName || 'Quiz'
      }));
    }
